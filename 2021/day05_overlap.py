...
class Line:
    def __init__(self, raw):
        ...
        self.diagonal = bool(self.delta_x and self.delta_y)
    ...
    def __iter__(self):
        yield from (
            (self.x1 + i * self.delta_x, self.y1 + i * self.delta_y)
            for i in range(1 + max(abs(self.x2 - self.x1), abs(self.y2 - self.y1)))
        )


def count(lines, diagonal=False):
    cnt = Counter(
        xy for line in lines if diagonal or line.diagonal == diagonal for xy in line
    )
