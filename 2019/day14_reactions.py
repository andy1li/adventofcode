...
def parse(input):
    reactions = {}
    for line in input:
        chemicals = re.findall(r'(\d+) ([A-Z]+)', line)
        amount, output = chemicals.pop()
        reactions[output] = int(amount), {
            chemical: int(amount)
            for amount, chemical in chemicals
        }
    return reactions

def filter_valid(needs):
    for chemical, amount in needs.copy().items():
...
if __name__ == '__main__':
    ...
    assert num_ore(parse(TEST2)) == 165
    assert num_ore(parse(TEST13312)) == 13312
    assert num_ore(parse(TEST180697)) == 180697
    assert num_ore(parse(TEST2210736)) == 2210736

    assert max_fuel(parse(TEST13312)) == 82892753
    assert max_fuel(parse(TEST180697)) == 5586022
    assert max_fuel(parse(TEST2210736)) == 460664

    reactions = parse(open('data/day14.in'))
    print(num_ore(reactions))
    print(max_fuel(reactions))